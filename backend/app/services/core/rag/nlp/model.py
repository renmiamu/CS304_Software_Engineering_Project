...
def rerank_similarity(query, texts):
    api_key = os.getenv("RERANK_API_KEY") or os.getenv("EMBEDDING_API_KEY") or os.getenv("DASHSCOPE_API_KEY")
    if not texts:
...
def generate_embedding(text: str | List[str], api_key: str = None, base_url: str = None, model_name: str = "text-embedding-v3", dimensions: int = 1024, encoding_format: str = "float", max_batch_size: int = 10):
    ...
    base_url = base_url or os.getenv("EMBEDDING_BASE_URL") or os.getenv("DASHSCOPE_BASE_URL") or "https://dashscope.aliyuncs.com/compatible-mode/v1"
