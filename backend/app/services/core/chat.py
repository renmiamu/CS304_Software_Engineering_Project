...
from app.services.web_search.web_search import serper_images, serper_videos
load_dotenv()
logger = logging.getLogger(__name__)

def _build_llm_client(model_name: str) -> OpenAI:
    """
    按模型名称选择对应供应商配置，避免 key 与 base_url 不匹配。

    约定：
    - deepseek-* 模型：优先使用 DEEPSEEK_API_KEY / DEEPSEEK_BASE_URL
    ...
    if not api_key:
        raise HTTPException(status_code=500, detail=f"Missing API key for model: {model_name}")
...
请直接返回JSON，不要包含其他文字。
    ...
    try:
        # 调用大模型生成推荐问题
        client = _build_llm_client("deepseek-chat")
...
def _truncate_text(text_value: str, max_length: int) -> str:
    """截断文本，避免上下文过长"""
    if not text_value:
        return ""
    if len(text_value) <= max_length:
        return text_value
    return text_value[:max_length] + "...(已截断)"

def get_short_term_memory(session_id: str) -> str:
    """
    获取会话短期记忆（最近 N 轮对话）并格式化为文本。
    默认读取最近 3 轮，可通过 SHORT_TERM_MEMORY_TURNS 配置。
    """
    try:
        memory_turns = 3
        ...
        for idx, row in enumerate(rows, start=1):
            history_question = _truncate_text(row.user_question or "", max_question_len)
            history_answer = _truncate_text(row.model_answer or "", max_answer_len)
            memory_lines.append(
                f"第{idx}轮\n用户: {history_question}\n助手: {history_answer}"
            )

        memory_text = "\n\n".join(memory_lines)
        logger.info(f"短期记忆加载成功，session_id={session_id}，轮数={len(rows)}")
        return memory_text
    except Exception as e:
        logger.warning(f"加载短期记忆失败，session_id={session_id}，error={str(e)}")
        return ""

def update_session_name(session_id: str, question: str, user_id: str):
    """
    根据 session_id 维护会话名称：
    1) 不存在则创建并写入 AI 生成名称
    2) 已存在但仍是默认名称(session-*)则更新为 AI 生成名称
    3) 已存在且非默认名称则跳过
    ...
    try:
        ...
        if not question:
            logger.info(f"Question is empty for session {session_id}, skipping session name update.")
            return

        if query_result:
            current_session_name = query_result.session_name

            # 只有默认名称才触发一次 AI 重命名
            if current_session_name and current_session_name.startswith("session-"):
                ...
                db.commit()
                logger.info(f"Session {session_id} renamed from default name to: {session_name}")
            else:
                logger.info(f"Session {session_id} already has non-default name, skipping.")
        else:
            session_name = generate_session_name(question)
            db.execute(
                text(
            ...
            db.commit()
            logger.info(f"New session {session_id} inserted with AI-generated name: {session_name}")
...
def get_chat_completion(session_id, question, retrieved_content, user_id, chat_model: str = "deepseek-chat"):
    """
...
请基于以上信息提供专业、准确的回答。
    ...
    try:
        # 初始化 OpenAI 客户端
        client = _build_llm_client(chat_model)
        ...
        for chunk in completion:
            if chunk.choices[0].finish_reason == "stop":
                ...
                if delta_content:
                    model_answer += delta_content  # 累加大模型的回答
                    message = {
                        "role": "assistant",
...
def get_chat_completion_with_search(session_id, question, retrieved_content, user_id, final_prompt, related_questions, snippets):
    """
    获取流式聊天完成结果，并按照指定格式输出。
    ...
    try:
        ...
        client = _build_llm_client(model_name)
