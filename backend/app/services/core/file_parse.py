...
import xxhash
import datetime
from app.services.core.rag.app.naive import chunk
from app.services.core.rag.utils.es_conn import ESConnection
from app.services.core.rag.nlp.model import generate_embedding
...
if __name__ == "__main__":
    project_base = get_project_base_directory()
    file_path = os.getenv(
        "RAG_TEST_FILE_PATH",
        os.path.join(project_base, "storage", "file", "sample.pdf"),
    )
    session_id = "40e2743ccffa4207"
    output_file = os.getenv(
        "RAG_TEST_OUTPUT_PATH",
        os.path.join(project_base, "storage", "output", "result.json"),
    )

    if not os.path.exists(file_path):
        raise FileNotFoundError(
            f"测试文件不存在: {file_path}。请设置环境变量 RAG_TEST_FILE_PATH 指向真实文件。"
        )
